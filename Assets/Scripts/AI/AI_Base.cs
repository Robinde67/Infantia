...
public class AI_Base : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		if(m_xPos != m_xGoal)
		{
			...
			if (m_xPos != m_xGoal){
				Debug.Log ("Current xPos: " + m_xPos.x + "|" + m_xPos.y);
				Instantiate(templight, new Vector3(m_xPos.x, m_xPos.y, -3.0f), transform.rotation);
			}
			
			//gameObject.transform.position = m_xGrid.test[(int)m_xPos.x][(int)m_xPos.y].transform.position;
			gameObject.transform.position = m_xPos;
		}
	}
	...
	void move (Vector2 p_fVelocity) {
		...
		gameObject.transform.position.Set(m_xGrid.test[(int)m_xPos.x + (int)p_fVelocity.x][(int)m_xPos.y + (int)p_fVelocity.y].transform.position.x,
